#!/usr/bin/env bash
# The tree does not re-download itself, and uploads have limits.
set -u
...
TOK=$(register poller@t.local "Poller" secret123)
promote poller@t.local
for i in 1 2 3 4 5; do
  req POST /api/admin/persons "$TOK" "{\"firstName\":\"Person$i\",\"lastName\":\"Test\"}"
done

echo "── the tree only re-sends when it changed ────────────────────────"
...
curl -s -X POST $API/api/admin/persons -H "Authorization: Bearer $TOK" \
  ...
  -d '{"firstName":"Newborn","lastName":"Test"}' >/dev/null
...
echo
echo "── uploads have a ceiling and a whitelist ────────────────────────"
